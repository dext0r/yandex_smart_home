...
from .const import (
    ...
    CONF_SKILL_ID, CONF_SKILL_USER_ID, CONF_ENTITY_CONFIG, CONF_FILTER
)
...
class YandexSkillLight():
    ...
    def __init__(self, hass: HomeAssistantType, config):
        ...
        self.skill_id = config[CONF_SKILL][CONF_SKILL_ID]
        self.user_id = config[CONF_SKILL][CONF_SKILL_USER_ID]
        ...
        try:        
            ...
            if self.oauth_token == '':
                ...
                return False
            _LOGGER.info(f"OAuth Token: {self.oauth_token}") # info
            if self.skill_id == '':
                _LOGGER.error("Async Init Failed: No skill ID")
                return False
            _LOGGER.info(f"Skill ID: {self.skill_id}") # info
            if self.user_id == '':
