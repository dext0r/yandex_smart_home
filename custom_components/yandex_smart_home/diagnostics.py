...
from custom_components.yandex_smart_home.device import async_get_device_states, async_get_devices
...
async def async_get_config_entry_diagnostics(hass: HomeAssistant, config_entry: ConfigEntry) -> dict[str, Any]:
    ...
    for device in await async_get_devices(hass, entry_data):
        diag["devices"][device.id] = device

        try:
            diag["states"][device.id] = (await async_get_device_states(hass, entry_data, [device.id]))[0]
        except Exception as e:
            diag["states"][device.id] = e
