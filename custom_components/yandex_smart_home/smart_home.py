...
async def async_handle_message(
    hass: HomeAssistant, data: RequestData, action: str, message: dict[str, Any]
) -> dict[str, Any]:
    ...
    if handler is None:
        ...
        return {"request_id": data.request_id, "payload": {"error_code": ERR_INTERNAL_ERROR}}
    ...
    try:
        ...
        return {"request_id": data.request_id, "payload": {"error_code": err.code}}
    except Exception:
...
async def async_devices_query(hass, data, message):
    ...
    for device in message.get("devices", []):
        ...
        if state is None:
            devices.append({"id": entity_id, "error_code": ERR_DEVICE_UNREACHABLE})
            continue
...
async def async_devices_execute(hass: HomeAssistant, data: RequestData, message: DeviceActionRequest) -> dict[str, Any]:
    ...
    for device in request.payload.devices:
        ...
        devices.append({"id": entity_id, "capabilities": capabilities_result})
