...
def test_property_float_pressure(hass, yandex_pressure_unit, v):
    entry = MockConfigEntry(options={const.CONF_PRESSURE_UNIT: yandex_pressure_unit})
    config = MockConfig(entry=entry)
    state = State("sensor.test", "740", {ATTR_DEVICE_CLASS: SensorDeviceClass.PRESSURE})
    assert_no_properties(hass, config, state, PROPERTY_FLOAT, const.FLOAT_INSTANCE_PRESSURE)

    state = State(
        "sensor.test",
        "740",
        {ATTR_DEVICE_CLASS: SensorDeviceClass.PRESSURE, ATTR_UNIT_OF_MEASUREMENT: const.PRESSURE_UNIT_MMHG},
    )
    prop = get_exact_one_property(hass, config, state, PROPERTY_FLOAT, const.FLOAT_INSTANCE_PRESSURE)
    prop.state = State("sensor.test", "740", {ATTR_DEVICE_CLASS: SensorDeviceClass.PRESSURE})
    ...
    prop = get_exact_one_property(hass, config, state, PROPERTY_FLOAT, const.FLOAT_INSTANCE_PRESSURE)
    assert prop.retrievable
    assert prop.parameters() == {"instance": "pressure", "unit": PRESSURE_UNITS_TO_YANDEX_UNITS[yandex_pressure_unit]}
...
async def test_property_float_tvoc(hass, unit, v):
    ...
    state = State(
        "air_quality.test", STATE_ON, {"total_volatile_organic_compounds": 30, ATTR_UNIT_OF_MEASUREMENT: unit}
    )
...
async def test_property_float_amperage_value(hass, unit, v):
    state = State("switch.test", STATE_ON, {"current": 1245, ATTR_UNIT_OF_MEASUREMENT: unit})
...
@pytest.mark.parametrize(
    "domain,device_class,attribute,instance,unit,supported",
    [
