...
class ColorProfileMockConfig(MockConfig):
    ...
    def color_profiles(self) -> dict[str, dict[str, int]]:
        return {"test": {"red": ColorConverter.rgb_to_int(255, 191, 0), "white": 4120}}
...
async def test_capability_color_setting_rgb_with_internal_profile(hass, color_modes, features):
    config = ColorProfileMockConfig(
        entity_config={"light.test": {const.CONF_COLOR_PROFILE: "natural"}},
        entity_filter=generate_entity_filter(include_entity_globs=["*"]),
    )
...
async def test_capability_color_setting_temperature_k(hass, attributes, temp_range):
    ...
    assert cap.retrievable
    assert cap.parameters()["temperature_k"] == {"min": temp_range[0], "max": temp_range[1]}
    assert cap.get_value() is None
...
async def test_capability_color_setting_temprature_k_extend(hass):
    ...
    cap = get_exact_one_capability(hass, BASIC_CONFIG, state, CAPABILITIES_COLOR_SETTING, COLOR_SETTING_TEMPERATURE_K)
    assert cap.parameters()["temperature_k"] == {"min": 2700, "max": 6500}
    ...
    cap = get_exact_one_capability(hass, BASIC_CONFIG, state, CAPABILITIES_COLOR_SETTING, COLOR_SETTING_TEMPERATURE_K)
    assert cap.parameters()["temperature_k"] == {"min": 1500, "max": 9000}
    ...
    cap = get_exact_one_capability(hass, BASIC_CONFIG, state, CAPABILITIES_COLOR_SETTING, COLOR_SETTING_TEMPERATURE_K)
    assert cap.parameters()["temperature_k"] == {"min": 4500, "max": 4500}
    ...
    state = State("light.test", STATE_OFF, attributes)
    cap = get_exact_one_capability(hass, BASIC_CONFIG, state, CAPABILITIES_COLOR_SETTING, COLOR_SETTING_TEMPERATURE_K)
    assert cap.parameters()["temperature_k"] == {"min": 1500, "max": 7500}

    state = State(
        "light.test", STATE_OFF, dict({light.ATTR_COLOR_TEMP: color_temperature_kelvin_to_mired(2300)}, **attributes)
    )
    cap = get_exact_one_capability(hass, BASIC_CONFIG, state, CAPABILITIES_COLOR_SETTING, COLOR_SETTING_TEMPERATURE_K)
    assert cap.get_value() == 1500

    state = State(
        "light.test", STATE_OFF, dict({light.ATTR_COLOR_TEMP: color_temperature_kelvin_to_mired(6800)}, **attributes)
    )
    cap = get_exact_one_capability(hass, BASIC_CONFIG, state, CAPABILITIES_COLOR_SETTING, COLOR_SETTING_TEMPERATURE_K)
    assert cap.get_value() == 7500

    state = State(
        "light.test", STATE_OFF, dict({light.ATTR_COLOR_TEMP: color_temperature_kelvin_to_mired(6700)}, **attributes)
    )
    cap = get_exact_one_capability(hass, BASIC_CONFIG, state, CAPABILITIES_COLOR_SETTING, COLOR_SETTING_TEMPERATURE_K)
...
async def test_capability_color_setting_scene(hass):
    ...
    config = MockConfig(
        entity_config={
            state.entity_id: {
                const.CONF_ENTITY_MODE_MAP: {const.COLOR_SETTING_SCENE: {const.COLOR_SCENE_GARLAND: ["foo"]}}
            }
        }
    )
