...
async def test_property_custom(hass, domain, instance):
    ...
    if domain == binary_sensor.DOMAIN and instance in [
        ...
        with pytest.raises(SmartHomeError) as e:
            CustomEntityProperty.get(hass, BASIC_CONFIG, state, {const.CONF_ENTITY_PROPERTY_TYPE: instance})

        assert e.value.code == const.ERR_DEVICE_UNREACHABLE
        assert "Unsupported" in e.value.message
        return

    prop = CustomEntityProperty.get(hass, BASIC_CONFIG, state, {const.CONF_ENTITY_PROPERTY_TYPE: instance})
...
async def test_property_custom_get_value_float_conversion(hass, instance: str, unit, value):
    ...
    prop = CustomEntityProperty.get(
        ...
        BASIC_CONFIG,
        state,
        {const.CONF_ENTITY_PROPERTY_TYPE: instance, const.CONF_ENTITY_PROPERTY_UNIT_OF_MEASUREMENT: unit},
    )
