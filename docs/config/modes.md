...
* `Wake Up`, `Огоньки`, `Swing` и д.р. - значение атрибута объекта в Home Assistant, которое соответствует значению режима в УДЯ.
  Задавать лучше строками в кавычках. Узнать все поддерживаемые режимы можно в атрибутах объекта через (Настройки --> Инструменты разработчика --> [Состояния](https://my.home-assistant.io/redirect/developer_states/)). Допустимо использование списка значений.

## Автоматически определяемые режимы { id=internal }

!!! info "Автоматически обнаруженный режим можно [отключить](../advanced/capabilities/about.md#disable)"
