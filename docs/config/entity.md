...
### Неофициальные типы устройств { id=type-unofficial }

В УДЯ имеются типы устройств, которые уже работают, но пока официально не представлены в документации к API умного дома.

!!! warning "Использование этих типов строго не рекомендуется! Их использование может привести к тому, что устройство не будет добавлено в УДЯ или внезапно перестанет работать."

| Тип                 | Описание     |
| ------------------- | ------------ |
| `light.dimmable`    | Диммер       |
| `light.sconce`      | Бра          |
...
1. Алиса перестанет озвучивать состояние устройства
2. В карточке устройства будут отображаться раздельные кнопки для включение и отключения
...
## Раздельные кнопки вкл/выкл { id=split_on_off }

> Параметр: `split_on_off`

> Возможные значения: `true`

Включает раздельное отображение кнопок включения и отключения устройства. Параметр похож на `state_unknown`, однако УДЯ будет знать актуальное состояние устройства и Алиса сможет его озвучить.

!!! note "Параметр поддерживается с версии `v1.1.1`"
