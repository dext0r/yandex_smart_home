...
* Убедитесь, что:
     ...
     5. Устройство не находится в состоянии "Требует настройки" в списке всех устройств навыка (:fontawesome-solid-plus: в правом верхнем углу --> `Устройство умного дома` --> Выберите навык)
...
* **ID** и **атрибуты** проблемных объектов: их можно найти в `Настройки` --> `Инструменты разработчика` --> `Состояния`
...
Только для прямого подключения: обновите токен в параметрах навыка в [настройках интеграции](./config/getting-started.md#gui) и убедитесь, что указан верный Идентификатор навыка
