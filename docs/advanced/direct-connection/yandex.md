...
* Прописанный адрес Home Assistant в разделе `Настройки` --> `Система` --> `Сеть` --> `URL-адрес сервера` или в параметре [`external_url`](https://www.home-assistant.io/integrations/homeassistant/#external_url) в `configuration.yaml`.
...
* Зайдите на [dialogs.yandex.ru](https://dialogs.yandex.ru/developer/smart-home) и создайте навык Умного дома.
  Желательно это делать из под аккаунта, который планируется использовать для управления умным домом.
  При необходимости доступ к навыку можно предоставить другим пользователям Яндекса (раздел Доступ).

    ![](../../assets/images/direct-connection/skill.png){ width=750 }

* Заполните параметры (раздел `Основная информация`):
...
| Список поддерживаемых устройств | Любой текст                                                                                                                          |
...
* Нажмите кнопку `Сохранить` в начале страницы
* Заполните параметры (раздел `Связка аккаунтов`):
...
* Нажмите кнопку `Сохранить` в начале страницы
* Перейдите в раздел `Основная информация` и **нажмите** `Опубликовать` (для приватных навыков публикация автоматическая и моментальная).
...
* Найдите в списке и выберите производителя (ищите по названию <b>навыка</b>, который создали ранее)<br>
  ![](../../assets/images/direct-connection/add_dialog_home_assistant.jpg){ width=750 }
...
Удалите их оттуда вручную через в [настройках](https://dialogs.yandex.ru/developer/smart-home) навыка (вкладка `Тестирование`).
