...
* Если используется облачное подключение - попробуйте уменьшить количество добавляемых в УДЯ устройств через [фильтры](#фильтры). Особое внимание уделите домену `sensor`, как правило именно он является источником огромного количества ненужных устройств.
