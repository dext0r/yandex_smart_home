...
2. Через облако (бета-тест): настройка в несколько кликов, не требуется доступ к Home Assistant из интернета, полностью бесплатно
...
Для применения изменений из YAML перезагрузите интеграцию на странице "Настройки" -> "Интеграции", предварительно рекомендуется выполнить проверку конфигурации на странице "Настройки" > "Сервер" (для доступа включите "Расширенный режим" в профиле пользователя).
