...
* Зайти на [dialogs.yandex.ru](https://dialogs.yandex.ru) и создать диалог с типом "Умный дом".
...
| `channel_set_via_media_content_id` | `false` | `true` / `false`     | Только для домена `media_player`. Выбор канала через `media_content_id`, [подробнее...](https://github.com/dmitry-k/yandex_smart_home/issues/36) (скорее всего устарело)
...
Если маппинг не удался - управление функцией через УДЯ будет недоступно (справедливо после того, как закроется [#259](https://github.com/dmitry-k/yandex_smart_home/issues/259)).
...
Бинарные датчики (двери, утечка) и события (вибрация, нажатие кнопки) пока в бете и неподдерживаются на стороне УДЯ.
...
1. Если датчик представлен атрибутом устройства (например атрибут `water_level` у увлажнителя `humidifer.sample`) -
   достаточно отдать в УДЯ через фильтр `humidifer.sample`, уровень воды подхватится автоматически.
...
3. Датчик представлен отдельным устройством, но его требуется представить как датчик другого устройства.
   ...
   В этом случае дополнительные датчики и их типы задаются как элементы списка `properties` основного устройства:
  ```yaml
  yandex_smart_home:
    filter:
      include_entities:
  ...
  ```
  Возможные значения `type` смотрите в [документации УДЯ](https://yandex.ru/dev/dialogs/smart-home/doc/concepts/float-instance.html).
...
* Попробуйте обновить список устройств в УДЯ через [квазар](https://yandex.ru/quasar/iot):
  иконка "Добавить" -> "Устройство умного дома" -> В поиске найти ваш диалог -> "Обновить список устройств".
